...
function twic(par)
    ξ = range(-par.β-10.0, 0.0, length = par.Nξ); dξ = ξ[2] - ξ[1] # Computational domain (x)
    # Obtain leading-order solution
    c, u0 = leading_order(ξ, dξ, par)
    u1 = stability_shoot(dξ, par, c, u0, par.q)
    return vec(u0), vec(u1), vec(ξ)
end
...
function stability_shoot(dξ, par, c, u0, q)
    ...
    for j = 1:20
        ...
        ω = ω - f/d
        if abs(ω - ω_old) < 1e-6
            return bvp_stability(dξ, ω, c, q, u0, du0, par)
        end
    end
