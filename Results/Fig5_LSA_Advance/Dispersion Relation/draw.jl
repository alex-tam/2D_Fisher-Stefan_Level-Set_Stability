...
function draw_kappa()
    ...
    q3 = vec(readdlm("q-(0.5,0.0,0.0).csv")); ω3 = vec(readdlm("omega-(0.5,0.0,0.0).csv")) 
    q4 = vec(readdlm("q-(1.0,0.0,0.0).csv")); ω4 = vec(readdlm("omega-(1.0,0.0,0.0).csv")) 
    q5 = vec(readdlm("q-(2.0,0.0,0.0).csv")); ω5 = vec(readdlm("omega-(2.0,0.0,0.0).csv"))  
    # Plot data
    plot(q1, ω1, label = L"$\kappa = 0.1$", linewidth = 2, xlabel = L"$q$", ylabel = L"$\omega$", margin = 3mm, xlims = (0, 5.0), ylims = (-3,0.5), grid = true, legend=:bottomleft)
    plot!(q2, ω2, label = L"$\kappa = 0.2$", linewidth = 2)
    plot!(q3, ω3, label = L"$\kappa = 0.5$", linewidth = 2)
    plot!(q4, ω4, label = L"$\kappa = 1$", linewidth = 2)
    plot!(q5, ω5, label = L"$\kappa = 2$", linewidth = 2)
    savefig("Advance_Dispersion_kappa.pdf")
end
...
function draw_uf()
    ...
    # Plot data
    plot(q1, ω1, label = L"$u_f = 0.0$", linewidth = 2, xlabel = L"$q$", ylabel = L"$\omega$", margin = 3mm, xlims = (0, 5.0), ylims = (-1.5,0.5), grid = true, legend=:bottomleft)
    ...
    savefig("Advance_Dispersion_uf.pdf")
end
